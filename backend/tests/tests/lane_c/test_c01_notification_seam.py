...
async def test_only_first_vote_notifies(factory, db):
    ...
    # First vote (no prior Vote row) → stages a notification.
    ...
    # Record the vote, then a subsequent call must stay silent.
...
def test_extract_mentions_ignores_code_spans():
    ...
    assert extract_mentions("email bob@example.com") == set()  # not a mention
