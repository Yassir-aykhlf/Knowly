...
async def test_can_view_visibility_rules(factory):
    ...
    assert content.can_view(approved, None) is True           # approved: everyone
    assert content.can_view(held, None) is False              # held: not anonymous
    assert content.can_view(held, stranger) is False          # held: not a stranger
    assert content.can_view(held, author) is True             # held: the author
    assert content.can_view(held, admin) is True              # held: an admin
